...
async def start(client, message):
    ...
    [
        InlineKeyboardButton('sʜᴀʀᴇ ᴛʜɪs ʙᴏᴛ 🍁', url='https://t.me/share/url?url=https://telegram.me/BENZSearchRobot&text=%0Aᴛᴇʟᴜɢᴜ%20|%20ᴛᴀᴍɪʟ%20|%20ʜɪɴᴅɪ%20|%20ᴍᴀʟᴀʏᴀʟᴀᴍ%20|%20ᴋᴀɴɴᴀᴅᴀ%0A%0A%20%20%20%20%20ɪ%20ᴀᴍ%20ᴘᴏᴡᴇʀғᴜʟ%20ғɪʟᴛᴇʀ%20💥%20ɪ%20ᴄᴀɴ%20sʜᴀʀᴇ%20ᴍᴏᴠɪᴇs%20ᴀɴᴅ%20sᴇʀɪᴇs%20ᴡɪᴛʜᴏᴜᴛ%20ᴀɴʏ%20ᴀᴅs...')
            ...
            title = '@PanindiaFilmZ  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@PanindiaFilmZ  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@PanindiaFilmZ  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
