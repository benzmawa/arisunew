...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            InlineKeyboardButton("☀  sᴇʟᴇᴄᴛ ᴍᴏᴠɪᴇ ʟᴀɴɢᴜᴀɢᴇs  ☀", callback_data=f"languages#{key}")
        ])
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="☀️ sᴇʟᴇᴄᴛ ʟᴀɴɢᴜᴀɢᴇ", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="🔺 sᴇʟᴇᴄᴛ sᴇᴀsᴏɴs", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("🚀 sᴇɴᴅ ᴀʟʟ", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("☀️ sᴇʟᴇᴄᴛ ᴀɢᴀɪɴ", callback_data=f"seasons#{key}")
        ])
    else:
        btn = []
        btn.insert(0, [
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Join First Bro Then Click 😒", show_alert=True)
            return
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<b>\n\n<u>🍿 ʏᴏᴜʀ ᴍᴏᴠɪᴇ ғɪʟᴇs 👇</u></b>\n"
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        if settings["button"]:
            cap = f"<b>ᴛʜᴇ ʀᴇsᴜʟᴛs ғᴏʀ : {search}\n\nʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ : {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ : {message.chat.title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
        else:
            cap = f"<b>ᴛʜᴇ ʀᴇsᴜʟᴛs ғᴏʀ : {search}\n\nʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ : {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ : {message.chat.title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
            cap+="<b><u>🍿 ʏᴏᴜʀ ᴍᴏᴠɪᴇ ғɪʟᴇs 👇</u></b>\n\n"
            for file in files:
